...
async function getFibonacciFromServer(num) {
  if (num > 50) {
    ...
    document.getElementById("alert").appendChild(alertText).className =
      "text-danger p-absolute";
  ...
  fetch("http://localhost:5050/getFibonacciResults").then((response) => {
    response.json().then((data) => {
      ...
      for (let fResults of sortedArray) {
        ...
        fibonacciList.innerHTML = `The Fibonnaci of <strong>${
          fResults.number
        }</strong> is <strong>${
          fResults.result
        }</strong>. Calculated at: ${cDate.toString()}`;
